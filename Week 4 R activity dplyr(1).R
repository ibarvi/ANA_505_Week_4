...
students2<-rename(students, gender=Sex)
...
#TASK: replace the ? with the dataset name
males<-filter(students2, gender=="Male")
...
#TASK: replace the ? with the column name
females<-filter(students2, gender=="Female")
...
slice(males, 5:3)
names(students)
relocate(students, Eye, .after = last_col()) 
